...
const CameraScreen:React.FC<CameraScreenProps> = ({scanID, setFormData, formData}) => {
  ...
  const extractName = (result: any) => {
    const nameRegex = /Last Name\. First Name\. Given Names\. Middle Name*([\w\s,.-]+)/;  
    const match = result.match(nameRegex);
  ...
  const extractAddress = (result: any) => {
    ...
    const match = result.match(addressRegex);
    return match ? match[1] : 'Address not found';
  };
  ...
  const recognizeText = async (base64: string) => {
    try {
      ...
      const idNumber = extractIDNumber(text); //ALREADY WORKING. NUMERIC CHARACTERS ARE EXTRACTED
